/**
 * Bot Universal de Telegram para agendamento
 * 
 * Esta versão usa a abordagem universal para convites de calendário
 * que não depende de senhas de aplicativo temporárias do Gmail
 */
...
// Iniciar o bot
export async function startUniversalBot() {
  try {
    ...
    await bot.telegram.setMyCommands([
      ...
      { command: 'configurar_email', description: 'Configurar email para convites' },
      { command: 'eventos', description: 'Listar seus eventos agendados' },
      { command: 'apagar', description: 'Apagar um evento do calendário' }
    ]);
    
    await bot.launch();
    log('Bot Universal do Telegram iniciado com sucesso!', 'telegram');
...
export function stopUniversalBot() {
  bot.stop('SIGINT');
