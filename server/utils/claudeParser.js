...
export async function parseEventWithClaude(userMessage, userTimezone = 'America/Sao_Paulo') {
  try {
    const currentDate = new Date();
    const dateForPrompt = currentDate.toLocaleDateString('pt-BR', { timeZone: userTimezone });
    const weekdayForPrompt = currentDate.toLocaleDateString('pt-BR', {
...
- Hoje é ${dateForPrompt} no fuso ${userTimezone}
...
- Considere que hoje é ${weekdayForPrompt}
...
Responda APENAS em JSON:
{
  ...
  "isValid": true
    ...
    return {
      ...
      date: correctedDate || currentDate.toISOString().split('T')[0],
      hour: result.hour || 9,
...
export default parseEventWithClaude;
