...
5. Extract emails as 'attendees'. Transcription often garbles addresses. If a list of "Known guest emails" is provided below, you MUST choose the single best match from that list when the user clearly refers to inviting someone by email; never invent a different domain or local part. Never output junk addresses (e.g. one-letter local part like o@...). If nothing in the list fits, use only an email substring that visibly appears in the user message; if still unsure, use []. Prefer at most one attendee email unless the user clearly names multiple distinct people with email.
