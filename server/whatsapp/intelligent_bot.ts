 ...
 */
export async function processWhatsAppMessage(phone: string, message: string): Promise<{
  success: boolean;
  response: string;
  event?: WhatsAppEvent;
  error?: string;
  ...
  try {
    ...
    // Converter para DateTime (mesmo processamento do Telegram)
    const startDateTime = DateTime.fromJSDate(event.startDate, { zone: 'America/Sao_Paulo' });
    const endDateTime = event.endDate ? 
      DateTime.fromJSDate(event.endDate, { zone: 'America/Sao_Paulo' }) :
    ...
    const whatsappEvent: WhatsAppEvent = {
      ...
      description: event.description || `Evento criado via WhatsApp de ${phone}`,
      displayDate: startDateTime.toLocaleString(DateTime.DATETIME_FULL, { locale: 'pt-BR' }),
      calendarLinks: generateCalendarLinks(event.title, startDateTime, endDateTime, event.location)
    };
...
${whatsappEvent.location ? `📍 ${whatsappEvent.location}` : ''}
 ...
 */
function generateCalendarLinks(title: string, startDate: DateTime, endDate: DateTime, location?: string) {
  ...
  return {
    google: `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodedTitle}&dates=${formatForGoogle(startDate)}/${formatForGoogle(endDate)}&location=${encodedLocation}&details=${encodedDescription}`,
    
    outlook: `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodedTitle}&startdt=${formatForOutlook(startDate)}&enddt=${formatForOutlook(endDate)}&location=${encodedLocation}&body=${encodedDescription}`,
